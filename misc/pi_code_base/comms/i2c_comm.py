...
reg=[bat_reg,limit_reg]

def get_data():
    global bus,bat_reg,ADDR
    while True:
        reg_sel=random.choice(reg)
        try:
            data=bus.read_i2c_block_data(ADDR,reg_sel,2)
            if reg_sel==bat_reg:
                print('Received     Bat-1 : ',data[0],'   ','Bat-2 : ',data[1])
            elif reg_sel==limit_reg:
                print('Received     Limit-U : ',data[0],'   ','Limit-D : ',data[1])
            time.sleep(5)
        except:
